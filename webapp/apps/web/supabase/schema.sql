...
create policy "members can create workspace images"
...
values ('images', 'images', false, 10485760, array['image/jpeg', 'image/png', 'image/webp', 'image/gif'])
on conflict (id) do update set
  public = excluded.public,
  file_size_limit = excluded.file_size_limit,
  allowed_mime_types = excluded.allowed_mime_types;

create policy "members can upload workspace image objects"
  ...
  with check (
    ...
    and exists (
      select 1 from public.workspace_members wm
      where wm.workspace_id = split_part(name, '/', 1)::uuid and wm.user_id = auth.uid()
    )
  );

create policy "members can read workspace image objects"
